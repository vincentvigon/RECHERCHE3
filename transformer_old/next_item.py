...
class Preprocessor_sign:
    ...
    def __init__(self,seq_len,d_model,nb_alternation=4):
        self.seq_len=seq_len
        self.d_model=d_model

        ind=tf.cast(tf.range(seq_len),tf.float32)

        scale=tf.sqrt(tf.cast(d_model,tf.float32))
        alternation=[]
        for i in range(1,nb_alternation+1):
            alternation.append((-1)**( (ind//i) %2)/scale)
...
def test_preprocessor_sign():
    ...
    prepros=Preprocessor_sign(seq_len,d_model)
    print(prepros.alternation)

    d_input=2
    x=tf.random.uniform([batch_size,seq_len,d_input])
    x=prepros(x)
    print(x)
...
class Transformator(tf.keras.layers.Layer):
    ...
    def __init__(self, seq_len, num_layers, d_model, num_heads, dff, rate=0.1):
        ...
        self.prepros=Preprocessor_sign(seq_len,d_model)
...
def test_custom_schedule():
    ...
    plt.plot(temp_learning_rate_schedule(tf.range(10_000, dtype=tf.float32)))
...
class Trainer:
    ...
    def __init__(self, predictor, data_generator):
        ...
        self.predictor = predictor
        self.lr_fn = CustomSchedule(predictor.d_model, 300)
        self.optimizer = tf.keras.optimizers.Adam(self.lr_fn)
    ...
    def main_loop(self, minutes_per_epoch, nb_epoch):
        ...
        for epoch in range(nb_epoch):
            ...
            ind = tf.cast(tf.range(len(self.train_losses)), tf.float32)
            axs[0].plot(ind, self.train_losses)

            axs[0].set_yscale("log")

            axs[1].plot(ind, self.lr_fn(ind))
            plt.show()
...
def test_agent():
    ...
    predictor = Transformator(seq_len, num_layers=6, d_model=128, num_heads=8, dff=128, rate=0)

    trainer = Trainer(predictor, data_generator)
    try:
        trainer.main_loop(0.4, 8)
