...
#         self.sound = k.sqrt(param.gamma * p_over_rho)  # sound_speed
#
#         self.Flux_ = k.stack([self.rhoV, self.rhoVV + self.p, self.V * (self.E + self.p)], axis=2)
#
#     # nx+2
#
#     def get_Flux(self):
...
#         aug = self.k.stack([self.rho, self.rhoV, self.E, self.V, self.p, self.sound, self.Flux_[:, :, 1], self.Flux_[:, :, 2]], axis=2)
...
#
#
# def Flux_HLL(var: Var_burger, param: Param, k: K):
...
#
#
# def Flux_HLL_LR(var_L: Var_burger, var_R: Var_burger, param: Param, k: K):
...
#
#
# def Flux_HLLC(var: Var_burger, param: Param, k: K):
#     # vrai flux
#     W_L = var.get("w", "L")
#     W_R = var.get("w", "R")
#
...
#
#
# def Flux_HLLC_LR(var_L: Var_burger, var_R: Var_burger, param: Param, k: K):
#     # vrai flux
#     W_L = var_L.get("w", "L")
#     W_R = var_R.get("w", "R")
#
...
#
#
# def compute_solutions_order1(param, nb_t, W_init, k: K):
...
#         res.append(w)
#         var = Var_burger(w, param, k)
#         dF_num = compute_dFnum_order1(w, param, k, param.HLLC)
#
#         # if param.HLLC:
#         #     Fnum = Flux_HLLC(var, param, k)
#         # else:
...
#
#
# def time_step_order2(w, param, k: K):
#
#     # la version prolongée de w
#     if param.BC == Param.BC_periodic:
...
#
#     var_recon_L = Var_burger(w - limited_slopes / 2, param, k)
#     var_recon_R = Var_burger(w + limited_slopes / 2, param, k)
#
#     if param.HLLC:
...
#
#
# def compute_solutions_order2(param, nb_t, W_init, k: K):
#     res = []
#
#     w = W_init  # inutile de faire une copie, elle est faites dans le Var_burger() au moment du padding
#
#     for t in range(nb_t):
#         res.append(w)
#         W_1 = time_step_order2(res[-1], param, k)
#         W_2 = time_step_order2(W_1, param, k)
#         w = (res[-1] + W_2) / 2
#
#     return k.stack(res)
#
# def time_step_order3(w, param, k: K):
#
#     # la version prolongée de w
#     if param.BC == Param.BC_periodic:
#         W_ = periodic_padding(w, k, 1)
...
#
#     var_recon_L = Var_burger(w - limited_slopes_L / 2, param, k)
#     var_recon_R = Var_burger(w + limited_slopes_R / 2, param, k)
#
#     if param.HLLC:
#         Fnum = Flux_HLLC_LR(var_recon_R, var_recon_L, param, k)
#     else:
...
#     return w - dt_over_dx * (Fnum[:, 1:, :] - Fnum[:, :-1, :])
#
# def compute_solutions_order3(param, nb_t, W_init, k: K):
#     res = []
#
#     w = W_init  # inutile de faire une copie, elle est faites dans le Var_burger() au moment du padding
#
#     for t in range(nb_t):
#         res.append(w)
#         W_1 = time_step_order3(res[-1], param,k)
#         W_2 = time_step_order3(W_1, param, k)
#         W_2 = (3 * res[-1] + 1 * W_2) / 4
#         W_3 = time_step_order3(W_2, param, k)
#         w = (1 * res[-1] + 2 * W_3) / 3
#
#     return k.stack(res)
#
...
#     """"""
#     assert param.order==1, "ordre >1 à accélérer"
#
#     if is_coarse:
#         assert W_init.shape[1] == param.nx_coarse
#         return compute_solutions_coarse(param, nb_t, W_init, k)
#     else:
#         assert W_init.shape[1] == param.nx
#         if param.order == 1:
#             return compute_solutions_order1(param, nb_t, W_init, k)
#         elif param.order == 2:
...
#
# #
# # def compute_fine_solutions(genParam: GenParam, param: Param, nb_t, compute_coarse_solution=False):
# #     k = K("tf", 64)
# #     generator = FuncGenerator(genParam, param, k)
# #     W_init = generator.init_W()
# #     ti = time.time()
...
# #         W_coarse = projecter.projection_3D(W_init)
# #
# #     w = W_init
# #
# #     if param.order > 3:
...
# #         if param.order == 1:
# #             # Fnum = compute_Fnum_order1(w, param, k, HLLC)
# #             res_dFnum[t] = compute_dFnum_order1(w, param, k, param.HLLC)
# #         elif param.order == 2:
# #             # Fnum = compute_Fnum_order2(w, param, k, HLLC)
# #             res_dFnum[t] = compute_dFnum_order2(w, param, k, param.HLLC)
# #         elif param.order == 3:
# #             # Fnum = compute_Fnum_order3(w, param, k, HLLC)
# #             res_dFnum[t] = compute_dFnum_order3(w, param, k, param.HLLC)
# #
# #         dt_over_dx = param.dt_over_dx
...
# #     if compute_coarse_solution:
# #         for _ in range(nb_t):
# #             var = Var_burger(W_coarse, param, k)
...
#     k = K("np", 32)
#
#     param = Param(6000, batch_size=2, BC=Param.BC_reflexive)
#
...
#     nb_t = 5000
#
#     W_coarse = Projecter(param.nx_ratio, 32).projection_3D(w)
#     if k.kind == "np":
