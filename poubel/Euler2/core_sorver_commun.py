...
def _flux_with_diffu(param: Param,w_moins, w_plus, D)->tf.Tensor:
    F_mean=(param.F_fn(w_moins)+param.F_fn(w_plus))/2
    dw=w_plus-w_moins
    return F_mean-dw * D


def flux_with_diffu_order1(param:Param, w: tf.Tensor, model_D:Model,addi:bool)->tf.Tensor:
...
def flux_with_diffu_order2(param:Param, w: tf.Tensor,model_L:Model,addi:bool)->tf.Tensor:
    ...
    if model_L is not None:
        ...
        L_model= model_L.call(dW)  # nx+2+2p-shr = nx + 2

        w=w[:,p:-p,:]  #nx+4
        dw=dw[:,p:-p,:] #nx+3
        if addi:
            L=param.dx / 2*(L_model + minmod(dw[:, :-1, :], dw[:, 1:, :])) #nx+2
        else:
            L=param.dx / 2*L_model
    else:
    ...
    F_mean = (param.F_fn(w_a) + param.F_fn(w_b)) / 2
    D = param.D_fn_rusanov(w_a, w_b)/2  # nx+1

    return F_mean - (w_b - w_a) * D #nx+1
