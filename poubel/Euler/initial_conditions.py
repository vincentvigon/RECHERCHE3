...
def init_random_sod(param: Param, batch_size: int, minimum_E=0.05, maximal_jump_E=2, minimum_rho=0.05, maximal_jump_rho=2, span_V=1):
    k = K("tf", 32)
    rho = generate_Riemann_Problem(param, batch_size, k, minimum_rho, maximal_jump_rho)
    return rho[..., k.newaxis]


def init_single_contact(param: Param, batch_size: int, minimum_P=0.05, maximum_P=2, span_V=1, minimum_rho=0.05, maximal_jump_rho=2):
    k = K("tf", 32)
    rho = generate_Riemann_Problem(param, batch_size, k, minimum_rho, maximal_jump_rho)
    return rho[..., k.newaxis]
...
class FuncGenerator:
    ...
    def init_W(self):
        ...
        # V = np.random.uniform(-1, 1, batch_size)[:, np.newaxis]
        rhoV = rho * V
        """
