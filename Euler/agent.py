...
from Euler.param import Projecter, Param
from Euler.core_solver_commun import compute_solutions_accelerate, compute_solutions_nan_filtred, flux_with_diffu_order1, flux_with_diffu_order2, one_time_step, compute_solutions_with_slopes
...
class Agent(pop.Abstract_Agent):
    ...
    def _error(self, is_model):
        ...
        e_1, e_2, e_oo = tf.reduce_mean(diff_abs).numpy(), tf.sqrt(tf.reduce_mean(diff_abs) ** 2).numpy(), tf.reduce_max(diff_abs).numpy()
    ...
    def _predict(self, is_model, get_slopes=False):
        ...
        if get_slopes:
            return compute_solutions_with_slopes(self.param, nb_t, w_init, order, model, self.addi)
        else:
            return compute_solutions_accelerate(self.param, nb_t, w_init, order, model, self.addi)

    def predict(self, get_slopes=False):
        res_model = self._predict(True, get_slopes=get_slopes)
        res_classic = self._predict(False, get_slopes=get_slopes)
