...
class Difference_model_several_convo:
    ...
    def __init__(self, param: Param, kernel_size=4, conv_struct=(64, 32, 32), dense_struct=(32, 16), return_zeros=True):
        ...
        if self.return_zeros:
            final_layer = layers.Dense(self.param.n_equations, kernel_initializer="zeros", bias_initializer="zeros", activation=self.param.final_activation)
        else:
            final_layer = layers.Dense(self.param.n_equations, activation=self.param.final_activation)
...
class Difference_model_tricky_no_diff:
    ...
    def __init__(self, param: Param, window_size, censure=False, model_struct=model_struct_default, return_zeros=True):
        ...
        if self.param.flux_correction == "slopes":
            input_ = layers.Input([param.nx_coarse + window_size - 2, dim_features])  # remplacer None par un entier (ex: 20) pour faire des test où l'on voit les shapes
        else:
            input_ = layers.Input([param.nx_coarse + window_size - 1, dim_features])
        ...
        input = self.tricky_convo(input)
        current = input

        for layer in self.hiden_layers:
            current = layer(current)
        end = final_layer(current)
...
class Difference_model_addi_old:
    ...
    def _model_one_part(self):
        ...
        end_ = layers.Dense(self.param.n_equations, kernel_regularizer=regularizer, kernel_initializer="zeros", bias_initializer="zeros")(out_)
